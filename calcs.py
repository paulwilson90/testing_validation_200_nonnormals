...
def wind_correct_formulated(ULD, wind_comp, flap):
    ...
    if wind_comp < -10:  # if the wind is more than 10 knot tail, add 1.6% for every knot over 10t
        ...
        # print(ten_tail_ULD)
        # print((1 + ((abs(wind_comp) - 10) * 1.6) / 100))
...
def get_v_speeds(weight, flap, vapp_addit, ice, ab_fctr):
    ...
    xl = pd.ExcelFile('200_MELCDL_MULTIPLIERS.xlsx')
...
def vapp_corrections(abnormal_dist, vref_addit, wet_dry):
    """Apply Operational Correction
        Factor
        1.30 (Dry VREF+10) Every knot is 1.03
        1.30 (Wet VREF)
        1.50 (Wet VREF+10) Every knot is 1.02
        """
    if wet_dry == "Wet":
        percent_increase = 1.3 + (vref_addit * 0.02)
    else:
        percent_increase = 1 + (vref_addit * 0.03)
...
def get_wat_limit(temp, flap, ice_protection, bleed, pressure_alt, test_case):
    ...
    # No change to WAT limit if the ice protection is on for Q200

    if flap == "35":  # Assumption is that aircraft will continue to land at flap 35
        return 15650, MLDW, off_chart_limits
    if flap == "5" or flap == "0":  # Should be able to climb with no WAT limit at these flap settings
        return 15650, MLDW, off_chart_limits
